...
function themewing_promo1( $atts, $content = null ) {
   ...
   if ( isset($image_bg_color) && $image_bg_color ){
      $imgstyle .= 'background-color:'.esc_attr($image_bg_color).';';
   }   
   if ( isset($alignment) && $alignment ){
      $promoalign = 'style="text-align:'.esc_attr($alignment).';"';
   }      

   $html  = '<div class="shortcode-thw-promo1 '.esc_attr($class).'" '.$promoalign.'>';
      $html  .= '<div class="feature-img-icon-box clearfix">';
            if ($icon_type == 'icon_select') {
               if ( isset($icon_name) && $icon_name ) 
               {
                  $html  .= '<span class="feature-icon" '.$icon_style.'><i class="fa fa-'.esc_attr($icon_name).'" style="'.$iconstyle.'"></i></span>';
               }
            }
            elseif($icon_type == 'image_select'){
               if (isset($src_img) && $src_img) 
               {
                 $html  .= '<span class="feature-img" style="'.$imgstyle.'"><img class="img-responsive" src="'.$src_img[0].'" alt="'.esc_attr($feature_title).'"></span>';
               }
            }else{
               $html  .= __('Select Icon Type','themewing');
            }

            $html  .= '<div class="feature-img-content">';
...
add_shortcode('thw_promo1', 'themewing_promo1');
/*------------------------------------------------*/

vc_map(

   array(
      "name" => __("Themewing Promo 1", "themewing"),
      "base" => "thw_promo1",
      ...
      "params" => array(
         ...
         array(
            "type" => "colorpicker",
            "heading" => __("Image background Color", "themewing"),
